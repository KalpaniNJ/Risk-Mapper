...
class YearlyFrequencyAlgorithm(QgsProcessingAlgorithm):
    """
    Generate yearly frequency maps from subfolders.
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for root, dirs, files in os.walk(main_input_folder):
            for fname in files:
                if fname.lower().endswith('.tif'):
                    match = re.search(year_pattern, fname)
                    if match:
                        year = match.group(1)
                        full_path = os.path.join(root, fname)
                        rasters_by_year[year].append(full_path)
                        feedback.pushInfo(f"{fname} → year {year}")
                    else:
                        feedback.pushInfo(f"Skipping file (does not match YYYY-MM-DD): {fname}")

        # Process each year
        for year, rasters in sorted(rasters_by_year.items()):
            if len(rasters) == 0:
                continue

            feedback.pushInfo(f"Processing year {year} with {len(rasters)} rasters...")
            ...
            output_path = os.path.join(output_base_folder, f'{prefix}{year}.tif')
            ...
            if load_maps:
                rlayer = QgsRasterLayer(output_path, f"{prefix}{year}")
                if rlayer.isValid():
        ...
        feedback.pushInfo("All yearly frequency maps created successfully.")
    ...
    def createInstance(self):
        return YearlyFrequencyAlgorithm()
        
    def icon(self):
            plugin_dir = os.path.dirname(__file__)
            icon_path = os.path.join(plugin_dir, "icons", "yrfreq_icon.jpeg")
